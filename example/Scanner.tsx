...
const HEART_RATE_SERVICE_UUID =
  "0000180d-0000-1000-8000-00805f9b34fb".toLowerCase();
const HEART_RATE_MEASUREMENT_UUID =
  "00002A37-0000-1000-8000-00805f9b34fb".toLowerCase();
const BODY_SENSOR_LOCATION_UUID =
  "00002A38-0000-1000-8000-00805f9b34fb".toLowerCase();

const BATTERY_SERVICE_UUID =
  "0000180f-0000-1000-8000-00805f9b34fb".toLowerCase();
const BATTERY_CHARACTERISTIC_LEVEL_UUID =
  "00002a19-0000-1000-8000-00805f9b34fb".toLowerCase();

const CUSTOM_SERVICE_UUID =
  "AAE28F00-71B5-42A1-8C3C-F9CF6AC969D0".toLowerCase();
...
export default function Scanner() {
  ...
  const [scanServiceUUIDs, setScanServiceUUIDs] = useState<string>(
  ...
  const [connectedDeviceServiceUUIDs, setConnectedDeviceServiceUUIDs] =
    useState<string[]>([]);
  const [connectedDeviceCharacteristics, setConnectedDeviceCharacteristics] =
    useState<Record<string, string[]>>({});
  const [bleNotificationSubscription, setBleNotificationSubscription] =
    useState<boolean>(false);
  const [hrNotificationSubscription, setHrNotificationSubscription] =
    useState<boolean>(false);
  const [logs, setLogs] = useState<string[]>([]);
  const bleModule = useRef(
    createBle({
  ...
  const getConnectedDevices = () => {
    const devices = ble.instance.getConnectedDevices([
      HEART_RATE_SERVICE_UUID,
  ...
  const connectDevice = async (deviceId: string) => {
    try {
      ...
      const servicesWithCharacteristics =
        await ble.instance.getServicesWithCharacteristics(connectedId);
      logMessage(
        `4 Got ${servicesWithCharacteristics.length} services for ${connectedId}`,
      );
      setConnectedDeviceServiceUUIDs(
        servicesWithCharacteristics.map((s) => s.uuid),
      );
      resetScannedDevices();
  ...
  const findAndConnect = async () => {
    try {
      ...
      const connectedId = await ble.instance.findAndConnect(
        Platform.select({
  ...
  const readBodySensorLocation = async () => {
    ...
    const key =
      bodySensorLocation && map[bodySensorLocation]
        ? map[bodySensorLocation]
        : "Unknown";
  ...
  const sendCommand = async (command: "enable-led" | "disable-led") => {
    ...
    switch (command) {
      case "enable-led":
        ...
        console.log("Will enable led with command:", 0x0e, enableLedCommand);
        const result = await ble.instance.writeCharacteristic(
          connectedDeviceId,
          CUSTOM_SERVICE_UUID,
          TX_CHAR_UUID,
          enableLedCommand,
      ...
      case "disable-led":
        ...
        const disResult = await ble.instance.writeCharacteristic(
          connectedDeviceId,
          CUSTOM_SERVICE_UUID,
          TX_CHAR_UUID,
          disableLedCommand,
  ...
  const listenToBleNotifications = async () => {
    ...
    const sub = await ble.instance.subscribeToCharacteristic(
      connectedDeviceId,
      CUSTOM_SERVICE_UUID,
      RX_CHAR_UUID,
      (_, data) => {
  ...
  const checkConnection = (deviceId = connectedDeviceId) => {
    ...
    logMessage(
      `Device ${deviceId} is ${isConnected ? "connected" : "disconnected"}`,
    );
    return isConnected;
  };
      ...
      {!isEnabled && (
        <>
          ...
          <TouchableOpacity
            style={styles.button}
            onPress={() =>
              ble.instance.openSettings().catch((e) => {
                console.error(e);
              })
            }
          >
          ...
          {!connectedDeviceId && (
            <>
              ...
              <TextInput
                ...
                onChangeText={setScanServiceUUIDs}
              ></TextInput>
              {isScanning ? (
                ...
                >
                  ...
                  {scanResults.map((device) => (
                    ...
                    >
                      ...
                      <Text>
                        Manufacturer Data:{" "}
                        {JSON.stringify(device.manufacturerData)}
                      </Text>
                      <Text>
                        Service UUIDs: {JSON.stringify(device.serviceUUIDs)}
                      </Text>
                      <Text>
                        Is Connectable: {device.isConnectable.toString()}
                      </Text>
              ...
              >
                ...
                {connectedDeviceServiceUUIDs.map((s, i) => (
                  <View
                    ...
                    style={{
                      borderBottomWidth:
                        i === connectedDeviceServiceUUIDs.length - 1 ? 0 : 1,
                    }}
                  >
                    <Text>Service: {s}</Text>
                    ...
                    {s === BATTERY_SERVICE_UUID &&
                      connectedDeviceCharacteristics[s]?.includes(
                        BATTERY_CHARACTERISTIC_LEVEL_UUID,
                      ) && (
                        <TouchableOpacity
                          style={styles.button}
                    ...
                    {s === CUSTOM_SERVICE_UUID &&
                      connectedDeviceCharacteristics[s]?.includes(
                        ...
                        <>
                          <TouchableOpacity
                            style={styles.button}
                            onPress={() => sendCommand("enable-led")}
                          >
                            <Text>Enable LED</Text>
                          </TouchableOpacity>
                          <TouchableOpacity
                            style={styles.button}
                            onPress={() => sendCommand("disable-led")}
                          >
                      ...
                      connectedDeviceCharacteristics[s]?.includes(
                        ...
                        <>
                          {!bleNotificationSubscription && (
                            <TouchableOpacity
                              style={styles.button}
                              onPress={listenToBleNotifications}
                            >
                          ...
                          {bleNotificationSubscription && (
                            <TouchableOpacity
                              style={styles.button}
                              onPress={unlistenToBleNotifications}
                            >
                          ...
                          {!bleNotificationSubscription &&
                            !hrNotificationSubscription && (
                              ...
                              >
                                <Text>
                                  Listen to HR and Custom Notifications
                                </Text>
                              </TouchableOpacity>
                            )}
                        </>
                      )}
                    {s === HEART_RATE_SERVICE_UUID &&
                      connectedDeviceCharacteristics[s]?.includes(
                        HEART_RATE_MEASUREMENT_UUID,
                      ) && (
                        <>
                          {!hrNotificationSubscription && (
                            <TouchableOpacity
                              style={styles.button}
                              onPress={listenToHrNotifications}
                            >
                          ...
                          {hrNotificationSubscription && (
                            <>
                              ...
                              >
                                <Text>
                                  Try override listen to HR Notifications
                                </Text>
                              </TouchableOpacity>
                              <TouchableOpacity
                                style={styles.button}
                                onPress={unlistenToHrNotifications}
                              >
                          ...
                          <TouchableOpacity
                            style={styles.button}
                            onPress={readBodySensorLocation}
                          >
              ...
              .map((log, i) => (
                <Text
                  ...
                  style={{
                    ...
                    color: "#444",
                    borderBottomWidth: i === logs.length - 1 ? 0 : 1,
                    padding: 6,
                  }}
                >
                  {log}
